...
def createBuffer(inputfn, bufferDist):
    ...
    return os.path.abspath(outputBufferfn)
...
def createHillShade(input_dsm):
    ...
    subprocess.run(
        ["gdaldem", "hillshade", "-z", "2", input_dsm, hillshade,]
    )

    return os.path.abspath(hillshade)


def createContour(input_dsm):
    output_dir = os.path.join(os.getcwd(), r"/python_code_outputs")
    output_name = os.path.basename(input_dsm).split(".")[0]
    contour = os.path.join(output_dir, output_name + "contour.shp")
    subprocess.run(
        [
            "gdal_contour",
            "-f",
    ...
    return os.path.abspath(contour)
...
def main(inputfn, bufferDist, input_dsm):
    ...
    contour_path = createContour(input_dsm)

    print(
        "YOUR FINAL OUTPUTS ARE CREATED AT {}".format(
            os.path.join(os.getcwd(), r"/python_code_outputs")
        )
    )
    ...
    # Contour
    contour = gpd.read_file(contour_path)

    fig, ax = plt.subplots(1, figsize=(8, 12))

    # Plot dsm & buffer boundary
    show((dsm, 1), ax=ax)
    contour.plot(edgecolor="blue", ax=ax, label="Hot Spots", linewidth=0.2)
